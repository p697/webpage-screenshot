var screenshot = {
  ...
  captureDesktop: function () {
    chrome.tabs.create({
      url: chrome.extension.getURL("/editor.html#capture"),
    });
  },
        ...
        chrome.tabs.query({ active: true, currentWindow: true }, function (a) {
          ...
          chrome.pageCapture.saveAsMHTML({ tabId: a.id }, function (data) {
            screenshot.tryGetUrl(function (filename) {
              ...
              filename +=
                "-" +
                new Date().getHours().toString().twoDigits() +
                new Date().getMinutes().toString().twoDigits() +
                new Date().getSeconds().toString().twoDigits();
              filename += ".mhtml";
