...
var page = {
  ...
  setVars: function (cropData) {
    ...
    if (
      cropData &&
      cropData.y1 > $(window).scrollTop() &&
      cropData.x1 > document.body.scrollLeft
    ) {
  ...
  onRequest: function (mess, sender, callback) {
    ...
    if (mess.start) {
      ...
      if (cropData.y2 - cropData.y1 > maxDimensionForCanvas)
        cropData.y2 = cropData.y1 + maxDimensionForCanvas;
      if (cropData.x2 - cropData.x1 > maxDimensionForCanvas)
        cropData.x2 = cropData.x1 + maxDimensionForCanvas;
    ...
    if (mess.type == "takeCapture") {
      ...
      for (var i = 0; i < document.getElementsByTagName("meta").length; i++) {
        ...
        if (
          ...
          ans.description = a.getAttribute("content");
      }
      if (mess.start) {
        dectect_zoom();
      ...
      ans.top = parseInt(
        $(window).scrollTop() * zoomLevel() - cropData.y1 * zoomLevel(),
        10
      );
      ans.left = parseInt(
        document.body.scrollLeft * zoomLevel() - cropData.x1 * zoomLevel(),
        10
      );
...
var f = function () {
  ...
  if (
    ...
    var func = function () {
      $("textarea")
        .val(decodeURIComponent(location.hash.slice(1)))
        .prev()
        .hide();
    };
...
function dectect_zoom() {
  (function (e, t, n) {
    ...
    var e = function () {
      ...
      n = function () {
        var t =
          Math.round(100 * (screen.deviceXDPI / screen.logicalXDPI)) / 100;
        return {
          zoom: t,
      ...
      r = function () {
        var t =
          Math.round(
            100 * (document.documentElement.offsetHeight / window.innerHeight)
          ) / 100;
        return {
          zoom: t,
          devicePxPerCssPx: t * e(),
      ...
      f = function (e, t, n, r, i, s) {
        ...
        window.matchMedia
          ...
          : ((a = document.getElementsByTagName("head")[0]),
            ...
            (l.className = "mediaQueryBinarySearch"),
